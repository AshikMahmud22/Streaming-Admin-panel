...
export default function ThemeModal({ onClose, editingTheme }: ThemeModalProps) {
  ...
  const handleSave = async () => {
    ...
    setIsUploading(true);
    const tid = toast.loading(editingTheme ? "Updating Theme..." : "Uploading Theme...");
      ...
      <div className="bg-white dark:bg-gray-900 w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl relative animate-in zoom-in-95 duration-200">
        ...
        <div className="space-y-6">
          <div
            ...
            className="aspect-video md:w-8/12 mx-auto w-full border-4 border-dashed border-gray-200 dark:border-gray-800 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-all group relative overflow-hidden"
          >
            {preview ? (
              ...
              <div className="text-center">
                ...
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Select Theme Background</span>
              </div>
            )}
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs font-black text-gray-500 uppercase ml-1 tracking-wider">Theme Name</label>
              <input
                type="text"
                placeholder="Ex: Neon Nights"
              ...
              <div>
                ...
                <div className={`relative mt-1.5 flex items-center border rounded-2xl transition-all ${
                  ...
                  <input
                    ...
                    disabled={!isPremium}
                    placeholder="0"
                    className={`w-full pl-9 pr-4 py-4 rounded-2xl outline-none bg-transparent dark:text-white font-bold transition-all ${!isPremium ? "cursor-not-allowed" : ""}`}
          ...
          >
            {isUploading ? <Loader2 className="animate-spin" /> : editingTheme ? "Update Theme" : "Publish Theme"}
          </button>
        </div>
      </div>
    </div>
