...
export const GiftModal = ({ isOpen, onClose, onSubmit, initialData, isUploading }: GiftModalProps) => {
  const [formData, setFormData] = useState<Partial<Gift>>({ name: "", value: "", category: "Uncategorized" });
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md lg:pl-64">
      <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative border border-gray-100 dark:border-gray-800">
        ...
        <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-2xl mb-6">
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all ${assetType === "png" ? "bg-white dark:bg-gray-700 shadow-sm text-blue-600" : "text-gray-500"}`}
          >
          ...
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all ${assetType === "svga" ? "bg-white dark:bg-gray-700 shadow-sm text-blue-600" : "text-gray-500"}`}
          >
        ...
        <form className="space-y-4" onSubmit={(e: FormEvent) => { e.preventDefault(); onSubmit(formData, file); }}>
          <div className="space-y-1">
            ...
            <input type="text" placeholder="e.g. Diamond Ring" required className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl outline-none dark:text-white border border-transparent focus:border-blue-500 transition-all" value={formData.name || ""} onChange={(e) => setFormData({...formData, name: e.target.value})} />
          </div>

          <div className="space-y-1">
            <label className="text-[10px] font-black text-gray-400 ml-1 uppercase tracking-wider">Coin Value</label>
            <input type="number" placeholder="0" required className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl outline-none dark:text-white font-mono border border-transparent focus:border-blue-500 transition-all" value={formData.value || ""} onChange={(e) => setFormData({...formData, value: e.target.value})} />
          </div>

          <div className="space-y-1">
            <label className="text-[10px] font-black text-gray-400 ml-1 uppercase tracking-wider">Collection Category</label>
            <select className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl outline-none dark:text-white border border-transparent focus:border-blue-500 transition-all appearance-none cursor-pointer" value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})}>
          ...
          <button disabled={isUploading} className="w-full bg-blue-600 hover:bg-blue-700 text-white h-14 rounded-2xl font-black shadow-lg shadow-blue-500/25 disabled:bg-gray-400 transition-all mt-4 flex items-center justify-center">
